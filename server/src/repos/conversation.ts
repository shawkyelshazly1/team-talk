...
import { meilisearchMutations, meilisearchQueries } from "../meilisearch";
...
export const setConversationStatus = async (conversationId: string, status: (typeof statusEnum.enumValues)[number], topic: string) => {
    try {
        const updatedConversation = await db.update(conversation).set({ status, topic }).where(and(eq(conversation.id, conversationId), not(eq(conversation.status, "closed")))).returning();
